...
<Timeline steps={["submit", "浏览器形成 FormData", "React 调用函数 Action", "Action pending/Transition", "成功提交", "非受控字段 reset"]} />

<Experiment title="发布与保存草稿">
运行中间 Demo，用同一表单分别点击默认提交按钮和带 \`formAction\` 的按钮。观察两条 Action 收到相同表单快照，但执行不同业务意图。
</Experiment>

<DemoReference action="分别点击默认提交和 formAction 按钮" observe="比较两条 Action 收到的 FormData 快照与不同业务意图，以及 pending/完成后的表单行为。" />
