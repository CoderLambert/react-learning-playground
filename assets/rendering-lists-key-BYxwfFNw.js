...
在中间 Demo 中编辑某一行，再排序/插入/删除列表，对比 stable id key 与 index key。操作前先预测输入 state 会留在“数据项”还是“位置”。
</Experiment>

<DemoReference action="编辑一行后重排列表，并切换 key 策略" observe="观察局部 state 是否跟随正确实体；检查 DOM 顺序变化与组件身份变化的差异。" />

<Flow items={["map 产生兄弟元素", "key 标识同级身份", "下一次 render 按 type + key 匹配", "匹配则保留对应 state", "身份变化则创建/重置子树"]} />
...
<FurtherReading items={[{ label: "React: Rendering Lists", href: "https://react.dev/learn/rendering-lists" }, { label: "React: Preserving and Resetting State", href: "https://react.dev/learn/preserving-and-resetting-state" }]} />`;export{e as default};