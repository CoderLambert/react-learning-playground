...
`,(0,n.jsx)(s,{action:`修改商品名称/数量，并运行“模拟相同输入重复 Render 两次”`,observe:`观察 JSX 输出随当前输入变化，并比较相同输入下纯计算结果稳定、非纯计算结果漂移；本 Demo 不测量真实 render/commit 次数。`}),`
`,(0,n.jsx)(l,{items:[`React 触发 render`,`调用组件函数读取当前输入`,`JSX 形成 React 元素描述`,`React 比较结果`,`commit 必要的 DOM 变化`]}),`
`,(0,n.jsx)(f,{children:(0,n.jsxs)(r.p,{children:[`组件函数可以多次执行，执行次数不等于 DOM 更新次数。纯 render 让 React 能安全地重试、暂停或重复计算；副作用若藏在 render 中，就会把“可重复计算”变成不可预测的外部修改。这里描述的是 React 的 render 语义，不是本 Demo 的生命周期测量结果；真实 render 与 commit 的可观测实验留给 `,(0,n.jsx)(r.code,{children:`render-commit`}),` 章节。`]})}),`
`,(0,n.jsx)(o,{title:`render 中的 mutation 边界`,children:(0,n.jsx)(r.p,{children:`禁止修改 render 开始前已经存在的 props、state、context 或其他外部数据。但对本次 render 内新创建、尚未逃逸的局部对象做局部构造或 mutation 是允许的；关键边界不是“完全不能 mutation”，而是不要修改非局部、预先存在的值。`})}),`
...
`,(0,n.jsx)(i,{title:`在 render 中产生副作用`,children:(0,n.jsx)(r.p,{children:`不要在组件函数主体里发送请求、写 localStorage、启动定时器或修改 render 之前已存在的外部对象。事件导致的动作放进 Event Handler；因渲染结果需要与外部系统同步的逻辑才考虑 Effect。`})}),`
...
`,(0,n.jsx)(r.li,{children:`render 必须保持纯净；局部新建对象的构造不等于修改外部世界。`}),`
`,(0,n.jsx)(r.li,{children:`DOM 修改属于 commit，本 Demo 不负责测量 render/commit 生命周期。`}),`
