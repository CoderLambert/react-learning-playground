...
<Flow items={["确认真实性能问题","用 React Profiler / 浏览器 Performance 定位瓶颈","确认瓶颈是否来自 React render，并找出 identity consumer 或昂贵计算边界","决定是否需要 memo / useMemo / useCallback 等具体工具","确认 React Compiler 的编译覆盖与 diagnostics","在同一场景复测，只保留已测量有效的热点优化"]} />
...
- 在 Chapter 07 中，reference equality 是 identity 基础，Profiler 是测量入口，\`memo\` / \`useMemo\` / \`useCallback\` 是具体工具，Compiler 改变手工优化的责任分配，但不改变 React 语义。
</Summary>

<FurtherReading items={[{label:"React: Profiler",href:"https://react.dev/reference/react/Profiler"},{label:"React: Performance tracks",href:"https://react.dev/reference/dev-tools/react-performance-tracks"}]} />
`;export{e as default};