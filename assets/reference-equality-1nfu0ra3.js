...
JavaScript 对对象、数组和函数使用引用身份；两个结构相同的新对象仍不满足 \`Object.is(a, b)\`。React 的 dependency 比较、\`memo\` 默认 props 比较以及本章涉及的 \`useMemo\` / \`useCallback\` 缓存机制都建立在这个事实之上。
</MentalModel>
...
<Observation>
引用变化不是错误，它是 JavaScript 正常语义。只有当 identity 被某个边界消费——例如 memoized child、Hook dependency 或外部订阅 snapshot——它才成为架构信号。本课提供 Chapter 07 的 identity 基础，但引用变化本身不是应该优化的证据。
</Observation>
...
- 不可变更新用引用变化表达新值；稳定引用则应在确认 identity consumer 后作为优化手段使用。
- 先理解消费者，再决定是否缓存引用。
</Summary>

<FurtherReading items={[{label:"React: memo",href:"https://react.dev/reference/react/memo"},{label:"React: useMemo",href:"https://react.dev/reference/react/useMemo"},{label:"MDN: Object.is",href:"https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object/is"}]} />
`;export{e as default};