...
`,(0,n.jsx)(c,{title:`排序后检查编辑状态跟谁走`,children:(0,n.jsx)(r.p,{children:`在中间 Demo 中编辑某一行，再反转或删除列表，对比 stable id key 与 index key。操作前先预测输入 State 会跟随“数据项”还是“当前位置”。`})}),`
`,(0,n.jsx)(s,{action:`编辑一行后重排列表，并切换 key 策略`,observe:`观察局部 state 是否跟随正确实体；检查 DOM 顺序变化与组件身份变化的差异。`}),`
`,(0,n.jsx)(l,{items:[`map 产生兄弟元素`,`key 为同级元素提供稳定身份线索`,`下一次 render 时，React 根据树中位置、组件类型和显式 key 判断身份是否延续`,`身份延续时对应 State 保留`,`身份变化时子树重新创建`]}),`
...
`,(0,n.jsxs)(o,{title:`key 不会作为普通 prop 传给组件`,children:[(0,n.jsx)(r.p,{children:`key 只用于 React 判断元素身份。`}),(0,n.jsxs)(r.p,{children:[`如果 Row 自己也需要业务 ID，必须显式传入一个普通 prop，例如代码示例：
`,(0,n.jsx)(r.code,{children:`<Row key={item.id} itemId={item.id} />`})]}),(0,n.jsx)(r.p,{children:`不要在 Row 内期待 props.key。`})]}),`
