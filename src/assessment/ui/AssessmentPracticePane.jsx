...
import { deriveAssessmentView, formatAssessmentPositionLabel, formatAssessmentProgress } from "./assessmentViewModel.js";
...
function EvidenceButton({ evidence, index, onOpen }) {
  const lineLabel = evidence?.kind === "source" ? `L${evidence.startLine}${evidence.endLine !== evidence.startLine ? `–${evidence.endLine}` : ""}` : null;
  const label = evidence?.kind === "source" ? `${evidence.fileName} · ${lineLabel}` : `查看依据 ${index + 1}`;
...
export function AssessmentPracticePane({ session = null, currentIndex = 0, answer = null, feedback = null, submitting = false, onStart, onAnswerChange, onSubmit, onNext, onOpenEvidence, onRequestAiQuestions }) {
  const view = deriveAssessmentView({ session, currentIndex, feedback });
  if (view.kind === "empty") {
    const canStart = typeof onStart === "function";
    return <section className="min-w-0 p-4 sm:p-5" aria-labelledby="assessment-empty-title"><Card className="overflow-hidden"><CardHeader className="border-b border-[var(--border-subtle)] bg-[var(--bg-surface-secondary)]"><div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[var(--color-primary-light)] text-base font-bold text-[var(--color-primary)]">{canStart ? "✓" : "?"}</div><div className="min-w-0"><p className="m-0 text-xs font-semibold uppercase tracking-[0.12em] text-[var(--text-subtle)]">知识点评测</p><h3 id="assessment-empty-title" className="mt-1 mb-0 text-base font-bold text-[var(--text-main)]">{canStart ? "准备好检查理解了吗？" : "当前知识点暂无评测"}</h3></div></div></CardHeader><CardContent className="space-y-4 pt-5"><p className="m-0 text-sm leading-6 text-[var(--text-muted)]">{canStart ? "开始后会记录本轮答题进度；题目使用创建会话时的快照，不受后续题库修改影响。" : "可以让 AI 根据当前知识点、笔记和源码准备一组可直接作答的评测题。"}</p><div className="flex flex-col gap-2 sm:flex-row">{canStart && <Button onClick={onStart} className="w-full sm:flex-1">开始测试</Button>}<Button variant={canStart ? "outline" : "default"} onClick={onRequestAiQuestions} disabled={!onRequestAiQuestions} className="w-full sm:flex-1">{canStart ? "让 AI 补充题目" : "让 AI 帮我出题"}</Button></div></CardContent></Card></section>;
  }
