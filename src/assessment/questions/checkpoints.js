...
function buildItems(chapter, kind, prompts) {
  const prefix = kind === "question" ? "q" : "e";
  return freezeItems(prompts.map((prompt, index) => ({
    id: `ch${String(chapter).padStart(2, "0")}-${prefix}${String(index + 1).padStart(2, "0")}`,
    chapter,
    kind,
    prompt,
    origin: "builtin",
...
const checkpoint = (chapter, title, questions, exercises) => Object.freeze({
  chapter,
  title,
  questions: buildItems(chapter, "question", questions),
  exercises: buildItems(chapter, "exercise", exercises),
});

export const CHECKPOINTS = Object.freeze({
  1: checkpoint(1, "Chapter 01 · UI 与组件模型", [
  ...
  2: checkpoint(2, "Chapter 02 · 事件、State 与渲染模型", [
  ...
  3: checkpoint(3, "Chapter 03 · State 建模与状态架构", [
  ...
  4: checkpoint(4, "Chapter 04 · Ref、Effect 与 Escape Hatches", [
  ...
  5: checkpoint(5, "Chapter 05 · Forms 与 React 19 Actions", [
  ...
  6: checkpoint(6, "Chapter 06 · Suspense 与并发 UI", [
  ...
  7: checkpoint(7, "Chapter 07 · 性能模型与优化", [
  ...
  8: checkpoint(8, "Chapter 08 · 外部 Store 与第三方系统", [
  ...
  9: checkpoint(9, "Chapter 09 · Server State 与请求架构", [
  ...
  10: checkpoint(10, "Chapter 10 · Router 与页面架构", [
  ...
  11: checkpoint(11, "Chapter 11 · TypeScript、Testing 与 Accessibility", [
  ...
  12: checkpoint(12, "Chapter 12 · SSR、RSC 与 Framework", [
...
export function getBuiltinCheckpoint(chapter) {
  return CHECKPOINTS[chapter] ?? null;
}

export function getAllBuiltinItems() {
  return Object.values(CHECKPOINTS).flatMap(({ questions, exercises }) => [...questions, ...exercises]);
}
