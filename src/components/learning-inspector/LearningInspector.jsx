      ...
      <div className="learning-inspector-surface">
        <header className="learning-inspector-header">
          ...
          <div className="learning-inspector-actions">
            ...
            <button type="button" className="learning-inspector-icon-button" onClick={() => onOpenChange?.(false)} aria-label="关闭学习面板" title="关闭学习面板"><span aria-hidden="true">×</span></button>
          </div>
        </header>
        <div className="learning-inspector-tabs" role="tablist" aria-label="学习面板内容">
          {resolvedPanels.map((panel) => {
            ...
            return <button key={panel.id} id={`learning-inspector-tab-${panel.id}`} type="button" role="tab" className={`learning-inspector-tab ${selected ? "is-active" : ""}`} aria-selected={selected} aria-controls={`learning-inspector-panel-${panel.id}`} tabIndex={selected ? 0 : -1} onClick={() => onTabChange?.(panel.id)}>{panel.label ?? panel.id}</button>;
          })}
        </div>
