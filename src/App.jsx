  ...
  return (
    <div className="workbench-notes-pane">
      ...
      <NoteViewer
        ...
        components={MDX_TEACHING_COMPONENTS}
        onTocChange={setToc}
      />
    </div>
  );
...
export default function App() {
  ...
  const { demoId, selectDemo } = useDemoUrlState({
    learningUnits: demos,
    defaultDemoId: demos[0]?.id,
  });

  const currentDemo = useMemo(
    () => demos.find((demo) => demo.id === demoId) || demos[0],
    [demoId],
  );
  const currentLearningUnit = useMemo(
    () => (currentDemo ? toLearningUnit(currentDemo) : null),
    [currentDemo],
  );
  const currentCategory = useMemo(
    () => CATEGORIES.find((category) => category.id === currentDemo?.category),
    [currentDemo],
  );
