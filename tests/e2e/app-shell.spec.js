...
test.describe("application shell", () => {
  test("supports focused mode, category navigation, and all-demo mode", async ({ page, browserDiagnostics }) => {
    ...
    expect(await page.locator(".category-group").count()).toBe(14);

    const categoryGroups = page.locator(".category-group");
    for (let index = 0; index < await categoryGroups.count(); index += 1) {
      await categoryGroups.nth(index).locator("button.nav-item").first().click();
  ...
  test("switches through every registered demo without a runtime crash", async ({ page }) => {
    ...
    const labels = await page.locator("button.nav-item > span:first-child").allTextContents();
    expect(labels.length).toBeGreaterThan(50);

    for (const label of labels) {
      await openDemo(page, label.trim());
    }
  });
  ...
  test("searches titles, category names, and keywords and restores content when cleared", async ({ page }) => {
    ...
    await expect(page.locator("button.nav-item").filter({ hasText: "Props 基础与解构" })).toBeVisible();

    await search.fill("Forms");
    await expect(page.locator(".category-group-title").filter({ hasText: "Forms 与 React 19 Actions" })).toBeVisible();
    expect(await navItems.count()).toBeGreaterThan(0);

    await search.fill("Cleanup");
    await expect(page.locator("button.nav-item").filter({ hasText: "useEffect 正确用法" })).toBeVisible();
