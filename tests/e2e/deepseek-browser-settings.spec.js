...
test("users can configure their own DeepSeek key and choose V4 Pro", async ({ page }) => {
  ...
  await page.getByLabel("API Key", { exact: true }).fill("sk-browser-user");
  await page.getByRole("combobox", { name: "DeepSeek 模型", exact: true }).selectOption("deepseek-v4-pro");
...
test("API key defaults to session storage and can be cleared", async ({ page }) => {
  await openAiTab(page);
  await page.getByRole("button", { name: /配置 DeepSeek/ }).click();
  await page.getByLabel("API Key", { exact: true }).fill("sk-session-only");
  await page.getByRole("combobox", { name: "DeepSeek 模型", exact: true }).selectOption("deepseek-v4-flash");
  await page.getByRole("button", { name: "保存配置" }).click();
